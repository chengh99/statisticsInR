...
head(mlb11)
# basic scatterplot
# For example, explore relationship between hits and homeruns and check the patterns of these two 
# variables
ggplot(mlb11, aes(x=hits, y=homeruns)) + 
    geom_point()
...
ggplot(mlb11, aes(x=at_bats, y=runs)) + 
    ...
    plot_ss(x = at_bats, y = runs, data = mlb11) + geom_smooth(method=lm , color="red", se=FALSE)
```
...
# Use the `lm` function in R to fit the linear model (a.k.a. 
# regression line).
mruns <- lm(runs ~ homeruns, data = mlb11)
# Fit the model
summary(mruns)
```
...
cur_bat <- 5579
mlb11 %>%
    filter(at_bats == cur_bat) %>%
    summarise(runs - (-2789.2429 + 0.6305 * cur_bat))
```
...
```{r runs-vs-strikeouts}
# type your code for the Exercise here, and Knit
# Use the `lm` function in R to fit the linear model (a.k.a. 
# regression line).
mstr <- lm(runs ~ strikeouts, data = mlb11)
# Fit the model
summary(mstr)
# plot
plot_ss(x = strikeouts, y = runs, data = mlb11, showSquares = TRUE)
```

To prepare our next exercise, I decided to choose a positive correlated variable sets.
```{r run-vs-bat-avg}
mbat <- lm(runs ~ bat_avg, data = mlb11)
# Fit the model
summary(mbat)
# plot
...
```
```{r bat_avg-residual}
ggplot(mbat, aes(x= .fitted, y=.resid))+
    geom_point()+
    geom_hline(yintercept = 0, linetype = "dashed")+
    xlab("Fitted values")+
    ylab("Residuals")
```
```{r bat_avg-residual-prop}
ggplot(mbat, aes(sample=.resid))+
    stat_qq()
```
...
summary(m1)
```
Then take a look at our bat_avg and runs analysis
```{r pick-model-batavg}
# runs and bat_avg
summary(mbat)
```

###### Therefore, bat_avg model can explain 65.6% of variabilities, while at_bats can explain 37.29% (R-square)
...
# Look at each model for R-square
# at bats: model "m1" with R-square 0.3729
# hit: model "mhits" with R-square 0.6419
# wins: model "mwins" with R-square 0.361
# batting average: model "mbat" with R-square 0.6561
```
...
# I plan to use linear model and measure R-square "fit"
# First, new_obs
...
ggplot(mlb11, aes(x=new_onbase, y=runs))+
    geom_point()+
    stat_smooth(method = "lm", se = FALSE)
mbase <- lm(runs ~ new_onbase, data = mlb11)
summary(mbase)
```
...
```{r diag}
# type your code for the Exercise here, and Knit
# new_obs has the highest R-square 0.9349
ggplot(mobs, aes(x=.resid))+
    geom_histogram(binwidth = 10)
```
```{r new_obs-prob}
ggplot(mobs, aes(sample=.resid))+
    stat_qq()
```
