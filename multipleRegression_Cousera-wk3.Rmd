...
ggplot(data = evals, aes(x = evals$score)) +
...
# Categorical variables such as cls, rank, ethinical groups are also evaluated
ggplot(evals, aes(x=evals$age, y=evals$bty_avg)) +
    geom_point(shape=1) +    # Use hollow circles
    geom_smooth(method=lm)
```
```{r categorical-box-plot}
# A really basic boxplot.
ggplot(evals, aes(x=as.factor(evals$ethnicity), y=evals$bty_avg)) + 
...
```{r summary-model}
# type your code for the Exercise here, and Knit
gradeTeacher <- lm(score ~ bty_avg, data = evals)
summary(gradeTeacher)
...
###### Since Adjusted R-square is only 0.03293, avg_bty is not a significant predictor to teacher's score
...
ggplot(data = gradeTeacher, aes(x = .fitted, y = .resid)) +  
  geom_point() + 
...
<li> <b> 0.12 points higher than minority professors, all else held constant. </b> </li> 
