...
```{r sampling-dist}
# type your code for the Exercise here, and Run Document
# 15,000 sample distributions
ggplot(data = sample_means50, aes(x = x_bar)) +
  geom_histogram(binwidth = 10)

sample_means50 %>%
  summarise(samp15k_mean = mean(x_bar), samp15k_med = median(x_bar), 
            samp15k_sd = sd(x_bar), samp15k_iqr = IQR(x_bar),
            samp15k_min = min(x_bar), samp15k_max = max(x_bar),
            samp15k_q1 = quantile(x_bar, 0.25),  # first quartile, 25th percentile
            samp15k_q3 = quantile(x_bar, 0.75))  # third quartile, 75th percentile
