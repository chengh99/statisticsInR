...
samp %>%
  summarise(samp60_mean = mean(area), samp60_med = median(area), 
            samp60_sd = sd(area), samp60_iqr = IQR(area),
            samp60_min = min(area), samp60_max = max(area),
            samp60_q1 = quantile(area, 0.25),  # first quartile, 25th percentile
            samp60_q3 = quantile(area, 0.75))  # third quartile, 75th percentile
...
if (params$mu >= sample_ci$lower & params$mu <= sample_ci$upper) {
  print("Sample(60) CI captures real population mean")
} else {
  print("Sample(60) CI does not capture real population mean")
}
